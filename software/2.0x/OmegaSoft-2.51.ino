...
//Offsets for tuning 
int servoY_offset = -108;
int servoX_offset = -135;

//Position of servos through the startup function
int servoXstart = -1 * servoY_offset;
int servoYstart = -1 * servoX_offset;
...
int ledred = 0;    // LED connected to digital pin 9
int ledblu = 4;    // LED connected to digital pin 9
...
void setup(){
  ...
  analogWriteFrequency(2, 333);
  analogWriteFrequency(3, 333);
...
pwmY = -1 *((PIDY * servoY_gear_ratio) + servoX_offset);
...
String datastring = "";
 ...
 datastring += "Motor Thrust,";
 datastring += String((rocketmass * accel.getAccelZ_mss()) - (rocketmass / 10));
 datastring += ",";
